...
///////////////////////////////////////////////////////////////////////////////
#define interrupt_wait_for_uart_tx()	while(!S0CON_SB_TI0)		//Waits for UART TX interrupt flag
#define interrupt_wait_for_uart_rx()	while(!S0CON_SB_RI0)		//Waits for UART RX interrupt flag
#define interrupt_wait_for_rfrdy()		while(!IRCON_SB_RFRDY)		//Waits for RFRDY interrupt flag
#define interrupt_wait_for_rfirq()		while(!IRCON_SB_RFIRQ)		//Waits for RFIRQ interrupt flag
#define interrupt_wait_for_spi_2wire()	while(!IRCON_SB_SPI_2WIRE)	//Waits for SPI/2-WIRE interrupt flag
#define interrupt_wait_for_wuopirq()	while(!IRCON_SB_WUOPIRQ)	//Waits for wake-up on pin interrupt flag
#define interrupt_wait_for_miscirq()	while(!IRCON_SB_MISCIRQ)	//Waits for misc interrupt (X16IRQ, ADCIRQ, RNGIRQ) flag
#define interrupt_wait_for_tick()		while(!IRCON_SB_TICK)		//Waits for tick interrupt (X16IRQ, ADCIRQ, RNGIRQ) flag
#define interrupt_wait_for_tf2()		while(!IRCON_SB_TF2)		//Waits for TF2 (timer 2 overflow) interrupt flag
#define interrupt_wait_for_exf2()		while(!IRCON_SB_EXF2)		//Waits for TF2 (timer 2 external reload) interrupt flag
...
#define interrupt_unmask_gpint0_ifp()		reg_bits_set(INTEXP, INTEXP_GP_INT0_ENABLE)					//Unmasks GPINT0 interrupt from pin in INTEXP
#define interrupt_mask_gpint0_ifp()			reg_bits_clear(INTEXP, INTEXP_GP_INT0_ENABLE)				//Masks GPINT0 interrupt from pin in INTEXP
#define interrupt_unmask_gpint1_ifp()		reg_bits_set(INTEXP, INTEXP_GP_INT1_ENABLE)					//Unmasks GPINT1 interrupt from pin in INTEXP
#define interrupt_mask_gpint1_ifp()			reg_bits_clear(INTEXP, INTEXP_GP_INT1_ENABLE)				//Masks GPINT1 interrupt from pin in INTEXP
#define interrupt_unmask_gpint2_ifp()		reg_bits_set(INTEXP, INTEXP_GP_INT2_ENABLE)					//Unmasks GPINT2 interrupt from pin in INTEXP
#define interrupt_mask_gpint2_ifp()			reg_bits_clear(INTEXP, INTEXP_GP_INT2_ENABLE)				//Masks GPINT2 interrupt from pin in INTEXP
