...
memory_flash_return_t memory_flash_read_bytes(uint16_t start_address, uint16_t num_bytes_to_read, uint8_t * data_array)
{
	...
	//Check parameters
	if((num_bytes_to_read == 0) || (start_address > MEMORY_FLASH_END_ADDRESS) || ((start_address + num_bytes_to_read ) > MEMORY_FLASH_END_ADDRESS))
	{
		return MEMORY_FLASH_BAD_PARAMS;
	}

	//Read the entire array byte-by-byte
