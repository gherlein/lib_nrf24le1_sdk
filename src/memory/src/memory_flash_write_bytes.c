...
//
// Description:
//  Writes the specified values to the specified addresses in flash memory
//
// Parameters:
...
memory_flash_return_t memory_flash_write_bytes(uint16_t start_address, uint16_t num_bytes_to_write, uint8_t * data_array)
{
	...
	//Write the entire array byte-by-byte
	for(x = 0; x < num_bytes_to_write; x++)
	{
		if(memory_flash_write_byte(start_address + x, data_array[x]) != MEMORY_FLASH_OK)
		{
			return MEMORY_FLASH_BAD_PARAMS;
		}
	}
