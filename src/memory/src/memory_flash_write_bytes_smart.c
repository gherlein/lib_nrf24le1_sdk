...
memory_flash_return_t memory_flash_write_bytes_smart(uint16_t start_address, uint16_t num_bytes_to_write, uint8_t * data_array, uint8_t * page_buffer)
{
	...
	uint16_t i, page_start_offset, bytes_written = 0;
	bool erase_cur_page, write_cur_page;

	//Check parameters
	if((num_bytes_to_write == 0) || (start_address > MEMORY_FLASH_END_ADDRESS) || ((start_address + num_bytes_to_write ) > MEMORY_FLASH_END_ADDRESS))
	{
		return MEMORY_FLASH_BAD_PARAMS;
	}

	//The initial page start is the offset in the page that start_address is located, and the initial page
	// is the page in which start_address is located
	page_start_offset = start_address % MEMORY_FLASH_PAGE_SIZE;
	cur_page_num = (uint8_t)(start_address / MEMORY_FLASH_PAGE_SIZE);
	...
	while(bytes_written < num_bytes_to_write)
	{
		...
		for(i = page_start_offset; (i < MEMORY_FLASH_PAGE_SIZE) && (bytes_written < num_bytes_to_write); i++, bytes_written++)
		{
		...
		//Copy the RAM page back to the flash page if necessary
