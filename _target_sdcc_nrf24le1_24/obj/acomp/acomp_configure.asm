...
; This file was generated Sat Feb 18 15:30:44 2012
;--------------------------------------------------------
	.module acomp_configure
...
;--------------------------------------------------------
	...
	.globl _SP
	.globl _P0
	.globl _acomp_configure_PARM_2
	.globl _acomp_configure
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_acomp_configure_PARM_2:
	.ds 1
_acomp_configure_acomp_config_options_1_1:
	.ds 1
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'acomp_configure'
;------------------------------------------------------------
;adc_channel               Allocated with name '_acomp_configure_PARM_2'
;acomp_config_options      Allocated with name '_acomp_configure_acomp_config_options_1_1'
;------------------------------------------------------------
;	src/acomp_configure.c:54: void acomp_configure(uint8_t acomp_config_options, adc_channel_t adc_channel)
;	-----------------------------------------
;	 function acomp_configure
;	-----------------------------------------
_acomp_configure:
	ar7 = 0x07
	ar6 = 0x06
	...
	ar0 = 0x00
	mov	a,dpl
	mov	dptr,#_acomp_configure_acomp_config_options_1_1
	movx	@dptr,a
;	src/acomp_configure.c:56: adc_set_input_channel(adc_channel);
	mov	dptr,#_acomp_configure_PARM_2
	movx	a,@dptr
	mov	dpl,a
	lcall	_adc_set_input_channel
;	src/acomp_configure.c:59: if((acomp_config_options & (COMPCON_REF_SEL << ACOMP_CONFIG_OPTION_COMPCON_OFFSET_SHIFT)) != ACOMP_CONFIG_OPTION_REF_VDD)
	mov	dptr,#_acomp_configure_acomp_config_options_1_1
	movx	a,@dptr
	mov	r7,a
	jnb	acc.1,00102$
;	src/acomp_configure.c:61: ADCCON1 = (ADCCON1 & ~ACOMP_CONFIG_OPTION_ADCCON1_WRITE_MASK) | ((acomp_config_options >> ACOMP_CONFIG_OPTION_ADCCON1_OFFSET_SHIFT) & ACOMP_CONFIG_OPTION_ADCCON1_WRITE_MASK);
	mov	r6,_ADCCON1
	anl	ar6,#0xFC
	mov	a,r7
	swap	a
	...
	mov	_ADCCON1,r6
00102$:
;	src/acomp_configure.c:65: COMPCON = (COMPCON & ~ACOMP_CONFIG_OPTION_COMPCON_WRITE_MASK) | ((acomp_config_options >> ACOMP_CONFIG_OPTION_COMPCON_OFFSET_SHIFT) & ACOMP_CONFIG_OPTION_COMPCON_WRITE_MASK);
	mov	r6,_COMPCON
	anl	ar6,#0xE0
	anl	ar7,#0x1F
	mov	a,r7
