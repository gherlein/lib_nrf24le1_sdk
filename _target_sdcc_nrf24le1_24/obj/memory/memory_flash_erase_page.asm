...
;--------------------------------------------------------
	.module memory_flash_erase_page
...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_memory_flash_erase_page_page_num_1_1:
	.ds 1
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'memory_flash_erase_page'
;------------------------------------------------------------
;page_num                  Allocated with name '_memory_flash_erase_page_page_num_1_1'
;------------------------------------------------------------
;	src/memory_flash_erase_page.c:47: memory_flash_return_t memory_flash_erase_page(uint8_t page_num)
;	-----------------------------------------
;	 function memory_flash_erase_page
;	-----------------------------------------
_memory_flash_erase_page:
	ar7 = 0x07
	ar6 = 0x06
	ar5 = 0x05
	ar4 = 0x04
	...
	ar0 = 0x00
	mov	a,dpl
;	src/memory_flash_erase_page.c:50: if(page_num >= MEMORY_FLASH_NUM_PAGES)
	mov	dptr,#_memory_flash_erase_page_page_num_1_1
	movx	@dptr,a
	mov	r7,a
	cjne	r7,#0x24,00111$
00111$:
	jc	00102$
;	src/memory_flash_erase_page.c:52: return MEMORY_FLASH_BAD_PARAMS;
	mov	dpl,#0x01
	ret
00102$:
;	src/memory_flash_erase_page.c:56: interrupt_save_global_flag(PSW_SB_F0);
	mov	c,_IEN0_SB_GLOBAL
	mov	_PSW_SB_F0,c
;	src/memory_flash_erase_page.c:57: interrupt_control_global_disable();
	 clr _IEN0_SB_GLOBAL 
;	src/memory_flash_erase_page.c:60: memory_flash_enable_write_access();
	 setb _FSR_SB_WEN 
;	src/memory_flash_erase_page.c:63: FCR = page_num;
	mov	_FCR,r7
;	src/memory_flash_erase_page.c:66: memory_flash_wait_for_write_complete();
00103$:
	jb	_FSR_SB_RDYN,00103$
;	src/memory_flash_erase_page.c:67: memory_flash_disable_write_access();
	 clr _FSR_SB_WEN 
;	src/memory_flash_erase_page.c:70: interrupt_restore_global_flag(PSW_SB_F0);
	mov	c,_PSW_SB_F0
	mov	_IEN0_SB_GLOBAL,c
;	src/memory_flash_erase_page.c:72: return MEMORY_FLASH_OK;
	mov	dpl,#0x00
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
