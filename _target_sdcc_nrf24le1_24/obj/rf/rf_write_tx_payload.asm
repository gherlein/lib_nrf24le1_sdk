...
_rf_write_tx_payload_PARM_3:
	.ds 1
_rf_write_tx_payload_dataptr_1_1:
	.ds 3
;--------------------------------------------------------
...
;dataptr                   Allocated with name '_rf_write_tx_payload_dataptr_1_1'
;status                    Allocated with name '_rf_write_tx_payload_status_1_1'
;------------------------------------------------------------
;	src/rf_write_tx_payload.c:49: uint8_t rf_write_tx_payload(uint8_t * dataptr, uint16_t len, bool transmit)
;	-----------------------------------------
...
_rf_write_tx_payload:
	...
	mov	r3,dph
	mov	a,dpl
	mov	dptr,#_rf_write_tx_payload_dataptr_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,r3
	movx	@dptr,a
	...
	mov	a,r2
	movx	@dptr,a
;	src/rf_write_tx_payload.c:53: status = rf_spi_execute_command(RF_W_TX_PAYLOAD_NOACK, dataptr, len, false); //Write the payload
	mov	dptr,#_rf_write_tx_payload_dataptr_1_1
	movx	a,@dptr
	mov	r2,a
	inc	dptr
	...
	mov	r2,dpl
;	src/rf_write_tx_payload.c:56: if(transmit == true)
	mov	dptr,#_rf_write_tx_payload_PARM_3
	movx	a,@dptr
	mov	r3,a
	cjne	r3,#0x01,00102$
;	src/rf_write_tx_payload.c:58: rf_transmit();
	push	ar2
	lcall	_rf_transmit
	pop	ar2
00102$:
;	src/rf_write_tx_payload.c:61: return status; //Return the value of STATUS
	mov	dpl,r2
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
