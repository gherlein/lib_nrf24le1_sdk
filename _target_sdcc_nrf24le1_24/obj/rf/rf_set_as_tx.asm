...
;------------------------------------------------------------
;config                    Allocated with name '_rf_set_as_tx_config_1_1'
;------------------------------------------------------------
;	src/rf_set_as_tx.c:47: void rf_set_as_tx()
;	-----------------------------------------
...
_rf_set_as_tx:
	...
	ar1 = 0x01
	ar0 = 0x00
;	src/rf_set_as_tx.c:51: rf_read_register(RF_CONFIG, &config, 1); //Read the current CONFIG value
	mov	dptr,#_rf_read_register_PARM_2
	mov	a,#_rf_set_as_tx_config_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,#(_rf_set_as_tx_config_1_1 >> 8)
	movx	@dptr,a
	inc	dptr
	clr	a
	movx	@dptr,a
...
;	src/rf_set_as_tx.c:54: if((config & RF_CONFIG_PRIM_RX) == 0)
	mov	dptr,#_rf_set_as_tx_config_1_1
	movx	a,@dptr
	mov	r7,a
	jb	acc.0,00102$
;	src/rf_set_as_tx.c:56: return;
	ret
00102$:
;	src/rf_set_as_tx.c:59: rf_clear_ce(); //Clear the CE pin
	 clr _RFCON_SB_RFCE 
;	src/rf_set_as_tx.c:62: config &= (~RF_CONFIG_PRIM_RX);
	mov	dptr,#_rf_set_as_tx_config_1_1
	mov	a,#0xFE
	anl	a,r7
	movx	@dptr,a
;	src/rf_set_as_tx.c:63: rf_write_register(RF_CONFIG, &config, 1);
