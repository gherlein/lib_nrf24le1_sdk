...
_rf_power_up_param_PARM_2:
	.ds 1
_rf_power_up_param_rx_active_mode_1_1:
	.ds 1
;--------------------------------------------------------
...
;rx_active_mode            Allocated with name '_rf_power_up_param_rx_active_mode_1_1'
;------------------------------------------------------------
;	src/rf_power_up_param.c:50: void rf_power_up_param(bool rx_active_mode, uint8_t config)
;	-----------------------------------------
...
_rf_power_up_param:
	...
	ar1 = 0x01
	mov	a,dpl
	mov	dptr,#_rf_power_up_param_rx_active_mode_1_1
	movx	@dptr,a
;	src/rf_power_up_param.c:53: config |= RF_CONFIG_PWR_UP;
	mov	dptr,#_rf_power_up_param_PARM_2
	movx	a,@dptr
	orl	a,#0x02
	movx	@dptr,a
;	src/rf_power_up_param.c:54: rf_write_register(RF_CONFIG, &config, 1);
...
;	src/rf_power_up_param.c:57: if((config & RF_CONFIG_PRIM_RX) == 0)
	mov	dptr,#_rf_power_up_param_PARM_2
	movx	a,@dptr
	mov	r2,a
	jb	acc.0,00105$
;	src/rf_power_up_param.c:59: rf_clear_ce(); //Clear CE for TX mode
	 clr _RFCON_SB_RFCE 
	sjmp	00107$
00105$:
;	src/rf_power_up_param.c:64: if(rx_active_mode != false)
	mov	dptr,#_rf_power_up_param_rx_active_mode_1_1
	movx	a,@dptr
	mov	r2,a
	jz	00102$
;	src/rf_power_up_param.c:66: rf_set_ce();
	 setb _RFCON_SB_RFCE 
	mov	dptr,#0x0004
	lcall	_delay_us
	sjmp	00107$
00102$:
;	src/rf_power_up_param.c:70: rf_clear_ce();
	 clr _RFCON_SB_RFCE 
;	src/rf_power_up_param.c:74: while(!pwr_clk_mgmt_is_cclk_src_xosc16m()); //wait for the clock to get switched to the 16 MHz crystal, otherwise the RF can't be run
00107$:
	mov	a,_CLKLFCTRL
	jnb	acc.3,00107$
;	src/rf_power_up_param.c:76: delay_us(1); //Delay for power up
	mov	dptr,#0x0001
	ljmp	_delay_us
	.area CSEG    (CODE)
	.area CONST   (CODE)
