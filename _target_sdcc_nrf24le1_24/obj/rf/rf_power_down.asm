...
;------------------------------------------------------------
;config                    Allocated with name '_rf_power_down_config_1_1'
;------------------------------------------------------------
;	src/rf_power_down.c:47: void rf_power_down()
;	-----------------------------------------
...
_rf_power_down:
	...
	ar1 = 0x01
	ar0 = 0x00
;	src/rf_power_down.c:51: rf_clear_ce(); //Clear the CE pin
	 clr _RFCON_SB_RFCE 
;	src/rf_power_down.c:53: rf_read_register(RF_CONFIG, &config, 1); //Read the current value of the CONFIG register
	mov	dptr,#_rf_read_register_PARM_2
	mov	a,#_rf_power_down_config_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,#(_rf_power_down_config_1_1 >> 8)
	movx	@dptr,a
	inc	dptr
	clr	a
	movx	@dptr,a
...
;	src/rf_power_down.c:56: if((config & RF_CONFIG_PWR_UP) == 0)
	mov	dptr,#_rf_power_down_config_1_1
	movx	a,@dptr
	mov	r7,a
	jb	acc.1,00102$
;	src/rf_power_down.c:58: return;
	ret
00102$:
;	src/rf_power_down.c:62: config &= (~RF_CONFIG_PWR_UP);
	mov	dptr,#_rf_power_down_config_1_1
	mov	a,#0xFD
	anl	a,r7
	movx	@dptr,a
;	src/rf_power_down.c:63: rf_write_register(RF_CONFIG, &config, 1);
