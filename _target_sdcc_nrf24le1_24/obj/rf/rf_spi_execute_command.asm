...
_rf_spi_execute_command_PARM_4:
	.ds 1
_rf_spi_execute_command_instruction_1_1:
	.ds 1
_rf_spi_execute_command_status_1_1:
	.ds 1
;--------------------------------------------------------
...
;instruction               Allocated with name '_rf_spi_execute_command_instruction_1_1'
;status                    Allocated with name '_rf_spi_execute_command_status_1_1'
;------------------------------------------------------------
;	src/rf_spi_execute_command.c:50: uint8_t rf_spi_execute_command(uint8_t instruction, uint8_t * dataptr, uint16_t len, bool copydata)
;	-----------------------------------------
...
_rf_spi_execute_command:
	...
	ar1 = 0x01
	mov	a,dpl
	mov	dptr,#_rf_spi_execute_command_instruction_1_1
	movx	@dptr,a
;	src/rf_spi_execute_command.c:52: uint8_t status = instruction; //status writes the instruction, then reads the current STATUS value
	mov	dptr,#_rf_spi_execute_command_instruction_1_1
	movx	a,@dptr
	mov	r2,a
	mov	dptr,#_rf_spi_execute_command_status_1_1
	movx	@dptr,a
;	src/rf_spi_execute_command.c:54: rf_clear_csn(); //Clear CSN to start the transaction
	 clr _RFCON_SB_RFCSN 
;	src/rf_spi_execute_command.c:57: rf_spi_send_read(&status, 1, true);
...
;	src/rf_spi_execute_command.c:58: rf_spi_send_read(dataptr, len, copydata);
	...
	mov	b,r4
	lcall	_rf_spi_send_read
;	src/rf_spi_execute_command.c:60: rf_set_csn(); //Set CSN to end the transaction
	 setb _RFCON_SB_RFCSN 
;	src/rf_spi_execute_command.c:62: return status; //Return the value of STATUS
	mov	dptr,#_rf_spi_execute_command_status_1_1
	movx	a,@dptr
	mov	dpl,a
	ret
