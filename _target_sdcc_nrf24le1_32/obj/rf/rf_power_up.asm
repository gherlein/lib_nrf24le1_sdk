...
; This file was generated Mon Mar 26 13:48:43 2012
;--------------------------------------------------------
	.module rf_power_up
...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_rf_power_up_rx_active_mode_1_1:
	.ds 1
_rf_power_up_config_1_1:
	.ds 1
;--------------------------------------------------------
...
;------------------------------------------------------------
;rx_active_mode            Allocated with name '_rf_power_up_rx_active_mode_1_1'
;config                    Allocated with name '_rf_power_up_config_1_1'
;------------------------------------------------------------
;	src/rf_power_up.c:48: void rf_power_up(bool rx_active_mode)
;	-----------------------------------------
...
_rf_power_up:
	...
	ar0 = 0x00
	mov	a,dpl
	mov	dptr,#_rf_power_up_rx_active_mode_1_1
	movx	@dptr,a
;	src/rf_power_up.c:52: rf_read_register(RF_CONFIG, &config, 1); //Get the current value of the CONFIG register
	mov	dptr,#_rf_read_register_PARM_2
	mov	a,#_rf_power_up_config_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,#(_rf_power_up_config_1_1 >> 8)
	movx	@dptr,a
	inc	dptr
	clr	a
	movx	@dptr,a
...
;	src/rf_power_up.c:55: if((config & RF_CONFIG_PWR_UP) != 0)
	mov	dptr,#_rf_power_up_config_1_1
	movx	a,@dptr
	mov	r7,a
	jnb	acc.1,00105$
;	src/rf_power_up.c:57: while(!pwr_clk_mgmt_is_cclk_src_xosc16m()); //wait for the clock to get switched to the 16 MHz crystal, otherwise the RF can't be run
00101$:
	mov	a,_CLKLFCTRL
	jnb	acc.3,00101$
;	src/rf_power_up.c:59: return;
	ret
00105$:
;	src/rf_power_up.c:63: rf_power_up_param(rx_active_mode, config);
	mov	dptr,#_rf_power_up_rx_active_mode_1_1
	movx	a,@dptr
	mov	r6,a
