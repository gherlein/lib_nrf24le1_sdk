...
;------------------------------------------------------------
;dataptr                   Allocated with name '_rf_irq_clear_all_dataptr_1_1'
;------------------------------------------------------------
;	src/rf_irq_clear_all.c:47: void rf_irq_clear_all()
;	-----------------------------------------
...
_rf_irq_clear_all:
	...
	ar1 = 0x01
	ar0 = 0x00
;	src/rf_irq_clear_all.c:50: uint8_t dataptr = RF_STATUS_RX_DR | RF_STATUS_TX_DS | RF_STATUS_MAX_RT;
	mov	dptr,#_rf_irq_clear_all_dataptr_1_1
	mov	a,#0x70
	movx	@dptr,a
;	src/rf_irq_clear_all.c:52: rf_write_register(RF_STATUS, &dataptr, 1);
	mov	dptr,#_rf_write_register_PARM_2
	mov	a,#_rf_irq_clear_all_dataptr_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,#(_rf_irq_clear_all_dataptr_1_1 >> 8)
	movx	@dptr,a
	inc	dptr
	clr	a
	movx	@dptr,a
...
;	src/rf_irq_clear_all.c:54: sbit_clear(IRCON_SB_RFIRQ); //Clear the master interrupt
	 clr _IRCON_SB_RFIRQ 
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
