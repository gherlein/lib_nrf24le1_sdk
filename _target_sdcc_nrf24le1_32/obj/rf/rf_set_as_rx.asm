...
; This file was generated Mon Mar 26 13:48:46 2012
;--------------------------------------------------------
	.module rf_set_as_rx
...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_rf_set_as_rx_rx_active_mode_1_1:
	.ds 1
_rf_set_as_rx_config_1_1:
	.ds 1
;--------------------------------------------------------
...
;------------------------------------------------------------
;rx_active_mode            Allocated with name '_rf_set_as_rx_rx_active_mode_1_1'
;config                    Allocated with name '_rf_set_as_rx_config_1_1'
;------------------------------------------------------------
;	src/rf_set_as_rx.c:47: void rf_set_as_rx(bool rx_active_mode)
;	-----------------------------------------
...
_rf_set_as_rx:
	...
	ar0 = 0x00
	mov	a,dpl
	mov	dptr,#_rf_set_as_rx_rx_active_mode_1_1
	movx	@dptr,a
;	src/rf_set_as_rx.c:51: rf_read_register(RF_CONFIG, &config, 1); //Read the current CONFIG value
	mov	dptr,#_rf_read_register_PARM_2
	mov	a,#_rf_set_as_rx_config_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,#(_rf_set_as_rx_config_1_1 >> 8)
	movx	@dptr,a
	inc	dptr
	clr	a
	movx	@dptr,a
...
;	src/rf_set_as_rx.c:54: if((config & RF_CONFIG_PRIM_RX) != 0)
	mov	dptr,#_rf_set_as_rx_config_1_1
	movx	a,@dptr
	mov	r7,a
	jnb	acc.0,00102$
;	src/rf_set_as_rx.c:56: return;
	ret
00102$:
;	src/rf_set_as_rx.c:60: config |= RF_CONFIG_PRIM_RX;
	mov	dptr,#_rf_set_as_rx_config_1_1
	mov	a,#0x01
	orl	a,r7
	movx	@dptr,a
;	src/rf_set_as_rx.c:61: rf_write_register(RF_CONFIG, &config, 1);
...
;	src/rf_set_as_rx.c:64: if(rx_active_mode != false)
	mov	dptr,#_rf_set_as_rx_rx_active_mode_1_1
	movx	a,@dptr
	mov	r7,a
	jz	00104$
;	src/rf_set_as_rx.c:66: rf_set_ce();
	 setb _RFCON_SB_RFCE 
	mov	dptr,#0x0004
	ljmp	_delay_us
00104$:
;	src/rf_set_as_rx.c:70: rf_clear_ce();
	 clr _RFCON_SB_RFCE 
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
