...
;--------------------------------------------------------
	...
	.globl _SP
	.globl _P0
	.globl _memory_flash_read_byte_PARM_2
...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_memory_flash_read_byte_PARM_2:
	.ds 3
_memory_flash_read_byte_address_1_1:
	.ds 2
_memory_flash_read_byte_pcon_temp_1_1:
	.ds 1
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'memory_flash_read_byte'
;------------------------------------------------------------
;data_byte                 Allocated with name '_memory_flash_read_byte_PARM_2'
;address                   Allocated with name '_memory_flash_read_byte_address_1_1'
;pcon_temp                 Allocated with name '_memory_flash_read_byte_pcon_temp_1_1'
;------------------------------------------------------------
;	src/memory_flash_read_byte.c:47: memory_flash_return_t memory_flash_read_byte(uint16_t address, uint8_t * data_byte)
;	-----------------------------------------
;	 function memory_flash_read_byte
;	-----------------------------------------
_memory_flash_read_byte:
	ar7 = 0x07
	ar6 = 0x06
	ar5 = 0x05
	ar4 = 0x04
	...
	mov	r7,dph
	mov	a,dpl
	mov	dptr,#_memory_flash_read_byte_address_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,r7
	movx	@dptr,a
;	src/memory_flash_read_byte.c:52: if(address > MEMORY_FLASH_END_ADDRESS)
	mov	dptr,#_memory_flash_read_byte_address_1_1
	movx	a,@dptr
	mov	r6,a
	inc	dptr
	...
	subb	a,r7
	jnc	00102$
;	src/memory_flash_read_byte.c:54: return MEMORY_FLASH_BAD_PARAMS;
	mov	dpl,#0x01
	ret
00102$:
;	src/memory_flash_read_byte.c:58: pcon_temp = PCON;
	mov	dptr,#_memory_flash_read_byte_pcon_temp_1_1
	mov	a,_PCON
	movx	@dptr,a
;	src/memory_flash_read_byte.c:59: memory_movx_accesses_code_memory();
	orl	_PCON,#0x10
;	src/memory_flash_read_byte.c:63: *data_byte = *((__xdata uint8_t *)address);
	...
	mov	b,r5
	lcall	__gptrput
;	src/memory_flash_read_byte.c:66: PCON = pcon_temp;
	mov	dptr,#_memory_flash_read_byte_pcon_temp_1_1
	movx	a,@dptr
	mov	_PCON,a
;	src/memory_flash_read_byte.c:68: return MEMORY_FLASH_OK;
	mov	dpl,#0x00
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
