...
; This file was generated Wed Jun 27 14:30:11 2012
;--------------------------------------------------------
	.module w2_configure
...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_w2_configure_PARM_2:
	.ds 1
_w2_configure_w2_config_options_1_1:
	.ds 2
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'w2_configure'
;------------------------------------------------------------
;address_in_slave_mode     Allocated with name '_w2_configure_PARM_2'
;w2_config_options         Allocated with name '_w2_configure_w2_config_options_1_1'
;------------------------------------------------------------
;	src/w2_configure.c:65: void w2_configure(uint16_t w2_config_options, uint8_t address_in_slave_mode)
;	-----------------------------------------
;	 function w2_configure
;	-----------------------------------------
_w2_configure:
	ar7 = 0x07
	ar6 = 0x06
	...
	mov	r7,dph
	mov	a,dpl
	mov	dptr,#_w2_configure_w2_config_options_1_1
	movx	@dptr,a
	inc	dptr
	mov	a,r7
	movx	@dptr,a
;	src/w2_configure.c:67: w2_enable(); //Datasheet v1.6, p. 159 states that you must enable 2 wire before changing anything else
	orl	_W2CON0,#0x01
;	src/w2_configure.c:70: W2CON0 = (W2CON0 & ~W2_CONFIG_OPTION_W2CON0_MASK) | ((uint8_t)(w2_config_options >> W2_CONFIG_OPTION_W2CON0_OFFSET_SHIFT) & W2_CONFIG_OPTION_W2CON0_MASK);
	mov	a,_W2CON0
	mov	dptr,#_w2_configure_w2_config_options_1_1
	movx	a,@dptr
	mov	r6,a
	...
	mov	ar4,r6
	mov	_W2CON0,r4
;	src/w2_configure.c:71: W2CON1 = (W2CON1 & ~W2_CONFIG_OPTION_W2CON1_MASK) | ((uint8_t)(w2_config_options >> W2_CONFIG_OPTION_W2CON1_OFFSET_SHIFT) & W2_CONFIG_OPTION_W2CON1_MASK);
	mov	r5,_W2CON1
	anl	ar5,#0xDF
	mov	ar4,r7
	mov	a,#0x20
	...
	orl	a,r5
	mov	_W2CON1,a
;	src/w2_configure.c:73: if(!w2_is_in_master_mode())
	mov	a,_W2CON0
	jb	acc.1,00102$
;	src/w2_configure.c:75: W2SADR = address_in_slave_mode;
	mov	dptr,#_w2_configure_PARM_2
	movx	a,@dptr
	mov	_W2SADR,a
00102$:
;	src/w2_configure.c:79: if((w2_config_options & W2_CONFIG_OPTION_ENABLE) == W2_CONFIG_OPTION_DISABLE)
	mov	a,r6
	jb	acc.0,00104$
;	src/w2_configure.c:81: w2_disable();
	anl	_W2CON0,#0xFE
00104$:
;	src/w2_configure.c:88: interrupt_unmask_2wire_completed();
	orl	_INTEXP,#0x04
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
