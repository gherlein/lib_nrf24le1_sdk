...
; This file was generated Fri Jul 13 20:24:06 2012
;--------------------------------------------------------
	.module uart_configure_manual_baud_calc
...
_uart_configure_manual_baud_calc_PARM_2:
	.ds 2
_uart_configure_manual_baud_calc_uart_config_options_1_1:
	.ds 1
;--------------------------------------------------------
...
;uart_config_options       Allocated with name '_uart_configure_manual_baud_calc_uart_config_options_1_1'
;------------------------------------------------------------
;	src/uart_configure_manual_baud_calc.c:55: void uart_configure_manual_baud_calc(uint8_t uart_config_options, uint16_t s0rel_val_if_br_gen_or_th1_val_if_tmr1)
;	-----------------------------------------
...
_uart_configure_manual_baud_calc:
	...
	ar0 = 0x00
	mov	a,dpl
;	src/uart_configure_manual_baud_calc.c:58: if(uart_config_options & UART_CONFIG_OPTION_BIT_SMOD_SET)
	mov	dptr,#_uart_configure_manual_baud_calc_uart_config_options_1_1
	movx	@dptr,a
	mov	r7,a
	jnb	acc.2,00102$
;	src/uart_configure_manual_baud_calc.c:60: reg_bits_set(PCON, PCON_SMOD);
	orl	_PCON,#0x80
	sjmp	00103$
00102$:
;	src/uart_configure_manual_baud_calc.c:64: reg_bits_clear(PCON, PCON_SMOD);
	mov	r6,_PCON
	anl	ar6,#0x7F
	mov	_PCON,r6
00103$:
;	src/uart_configure_manual_baud_calc.c:68: if(((uart_config_options & UART_CONFIG_OPTION_MODE_MASK) == UART_CONFIG_OPTION_MODE_1_UART_8_BIT) ||
	mov	a,#0xC0
	anl	a,r7
	mov	r6,a
	cjne	r6,#0x40,00116$
	sjmp	00107$
00116$:
;	src/uart_configure_manual_baud_calc.c:69: ((uart_config_options & UART_CONFIG_OPTION_MODE_MASK) == UART_CONFIG_OPTION_MODE_3_UART_9_BIT))
	mov	a,#0xC0
	anl	a,r7
	mov	r6,a
	cjne	r6,#0xC0,00108$
00107$:
;	src/uart_configure_manual_baud_calc.c:72: if((uart_config_options & UART_CONFIG_OPTION_CLOCK_MASK) == UART_CONFIG_OPTION_CLOCK_FOR_MODES_1_3_USE_BR_GEN)
	mov	a,#0x02
	anl	a,r7
	mov	r6,a
	cjne	r6,#0x02,00105$
;	src/uart_configure_manual_baud_calc.c:74: S0REL = s0rel_val_if_br_gen_or_th1_val_if_tmr1; //set S0REL to the value requested
	...
	movx	a,@dptr
	mov	(_S0REL >> 8),a
;	src/uart_configure_manual_baud_calc.c:75: sbit_set(ADCON_SB_BD); //enable BR generator
	 setb _ADCON_SB_BD 
	sjmp	00108$
00105$:
;	src/uart_configure_manual_baud_calc.c:79: sbit_clear(ADCON_SB_BD); //disable BR generator and use TMR1 overflow
	 clr _ADCON_SB_BD 
;	src/uart_configure_manual_baud_calc.c:85: (uint8_t)s0rel_val_if_br_gen_or_th1_val_if_tmr1);
	mov	dptr,#_uart_configure_manual_baud_calc_PARM_2
	movx	a,@dptr
	mov	r5,a
	inc	dptr
	...
	pop	ar7
;	src/uart_configure_manual_baud_calc.c:87: timer1_run(); //enable the timer
	 setb _TCON_SB_TR1 
00108$:
;	src/uart_configure_manual_baud_calc.c:92: S0CON = uart_config_options & UART_CONFIG_OPTION_S0CON_MASK;
	mov	a,#0xF0
	anl	a,r7
	mov	_S0CON,a
	ret
